...
void fVerificarStatus(int i_mud, int j_mud, struct stHotel h[LIN][COL]); //verifica o statsu de um quarto
...
void fFazerCadastro (struct stHotel h[LIN][COL]){
	...
	do{
    printf("\nDigite o nome do cliente (utilize um traco para marcar os espacos): ");
    gets(h[i_mud-1][j_mud-1].nome);
    //LimpaEntrada();

    if(strlen(h[i_mud][j_mud].nome)>MAX_NOME-1){
        printf("O tamanho do nome excedeu o tamanho maximo para este campo. \nPor Favor, digite novamente\n");
    }

    }
    while(strlen(h[i_mud][j_mud].nome)>MAX_NOME-1);
...
int fID_Verificar(int i_mud, int j_mud, struct stHotel h[LIN][COL]) //verifica o ID (CPF) do cliente
{
	...
	printf("Digite o seu ID ou CPF (documento de identificacao unica): ");
	gets(idFornecido);
...
void fVerificarStatus(int i_mud, int j_mud, struct stHotel h[LIN][COL]){ //verifica o status de um determinado quarto
...
main(){
	...
	int i_new, j_new; //andar/apto para a mudança de status de um apartamento (i - linha - andar) (j - coluna - quarto)
...
while(1){ //equanto a pessoa não optar por sair
    ...
    switch(opcao){
        ...
        case 7: //verifica o status de um determinado quarto
        	fVerificarStatus(i_new,j_new,Hotel);
            break;
